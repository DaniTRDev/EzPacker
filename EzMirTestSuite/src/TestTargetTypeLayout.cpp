#include "../include/TestTargetTypeLayout.h"

size_t TestTargetTypeLayout::getPointerSizeInBytes() const { return 8; }

size_t TestTargetTypeLayout::getTypeAlignmentInBytes(const MirType *type) const
{
    if (!type)
        return 1;
...
size_t TestTargetTypeLayout::getTypeSizeInBytes(const MirType *type) const
{
    if (!type)
        return 0;
