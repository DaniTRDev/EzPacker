...
{
    MirInstructionOpCode m_opcode{ MirInstructionOpCode::INVALID }; ///< Opcode of the instruction under test.
    uint32_t m_flags{ 0 };      // MirInstructionFlags (IsCall, IsReturn, IsSigned, etc.)
    size_t m_operandCount{ 0 }; ///< Number of valid entries in the parallel operand arrays.
    std::array<MirType *, 6> m_types{
        nullptr, nullptr, nullptr, nullptr, nullptr, nullptr
    }; // Concrete MirType pointers for operands
    ...
    bool m_hasImm{ false };  ///< True when an immediate operand was found and m_immValue is meaningful.
};
...
struct alignas(8) LegalityResponse
{
    ...
    /// True when the instruction is natively supported and needs no rewrite.
    constexpr bool isLegal() const noexcept { return m_action == LegalizeActionKind::Legal; }

    /// True when no legalization strategy exists and a diagnostic should be emitted.
