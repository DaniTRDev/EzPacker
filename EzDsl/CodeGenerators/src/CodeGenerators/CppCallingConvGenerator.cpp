...
void CppCallingConvGenerator::emitHeader(CppSourceEmitter &emitter) const
{
    ...
    for (Symbol *sym : getSymbolTable()->getSymbols())
    {
...

        {
            ...
            emitter.indent();
            emitter.emitLine("explicit {}(class MirBuilderContext *ctx, class MirRegisterClass *defaultRegClass = nullptr);", className);
            emitter.emitLine("~{}() override = default;", className);
            emitter.emitBlankLine();
            emitter.emitLine("ArgumentLocationDesc getArgLoc(class MirType *type, class CallLoweringState *callState) override;");
            emitter.emitLine("ArgumentLocationDesc getReturnLoc(class MirType *type, class CallLoweringState *callState) override;");
            ...
            emitter.emitLine("void classify(MirType *type, std::pmr::vector<CallingConvTypeClass> &out) const override;");
            emitter.emitLine("const std::pmr::vector<MirRegisterRef> &getAllCalleeSavedRegs() override;");
            emitter.emitLine("const std::pmr::vector<MirRegisterRef> &getCalleeSavedRegs(class MirRegisterClass *_class) override;");
            emitter.emitLine("const std::pmr::vector<MirRegisterRef> &getAllCallerSavedRegs() override;");
            emitter.emitLine("const std::pmr::vector<MirRegisterRef> &getCallerSavedRegs(class MirRegisterClass *_class) override;");
            emitter.dedent();
            emitter.emitBlankLine();
...
void CppCallingConvGenerator::emitSource(CppSourceEmitter &emitter) const
{
    ...
    for (Symbol *sym : getSymbolTable()->getSymbols())
    {
        ...
        emitter.dedent();
        {
            ...
            for (size_t i = 0; i < file.m_calleeSaved.size(); ++i)
            {
                emitter.emitLine("m_allCalleeSaved.push_back(MirRegisterRef(m_defaultClass, resolveRegId(\"{}\", {}))); // {}", file.m_calleeSaved[i].m_node, i + file.m_callerSaved.size(), file.m_calleeSaved[i].m_node);
            }
            for (size_t i = 0; i < file.m_callerSaved.size(); ++i)
            {
                emitter.emitLine("m_allCallerSaved.push_back(MirRegisterRef(m_defaultClass, resolveRegId(\"{}\", {}))); // {}", file.m_callerSaved[i].m_node, i, file.m_callerSaved[i].m_node);
            }
        }
        emitter.emitBlankLine();
        ...
        emitter.emitLine("bool {}::doesStackGrowsDownwards() const {{ return {}; }}", className, growthDown ? "true" : "false");

        emitter.emitLine("const char *{}::getName() const {{ return \"{}\"; }}", className, file.m_name.m_node);
        emitter.emitLine("size_t {}::getStackAlignment() const {{ return {}; }}", className, file.m_stack.m_alignment.m_node);
        emitter.emitLine("size_t {}::getShadowSpaceSize() const {{ return {}; }}", className, file.m_stack.m_shadowSpace.m_node);
        ...
        emitter.emitLine("bool {}::consumesSretSlot() const {{ return {}; }}", className, consumesSret ? "true" : "false");

        if (file.m_stack.m_linkRegister.has_value())
        {
            size_t lrId = findRegId(file.m_stack.m_linkRegister->m_node);
            emitter.emitLine("std::optional<MirRegisterRef> {}::getLinkRegister() const {{ return MirRegisterRef(m_defaultClass, resolveRegId(\"{}\", {})); }}", className, file.m_stack.m_linkRegister->m_node, lrId);
        }
        else
        {
            emitter.emitLine("std::optional<MirRegisterRef> {}::getLinkRegister() const {{ return std::nullopt; }}", className);
        }

        size_t fpId = findRegId(file.m_stack.m_framePointer.m_node);
        size_t spId = findRegId(file.m_stack.m_stackPointer.m_node);
        emitter.emitLine("MirRegisterRef {}::getFramePointerReg() const {{ return MirRegisterRef(m_defaultClass, resolveRegId(\"{}\", {})); }}", className, file.m_stack.m_framePointer.m_node, fpId);
        emitter.emitLine("MirRegisterRef {}::getStackPointerReg() const {{ return MirRegisterRef(m_defaultClass, resolveRegId(\"{}\", {})); }}", className, file.m_stack.m_stackPointer.m_node, spId);

        emitter.emitLine("bool {}::hasFramePointer(MirFunction *func) const {{ return func && func->getStackFrame() && func->getStackFrame()->getAllocatedObjectCount() > 0; }}", className);
        emitter.emitBlankLine();
        ...
        emitter.emitLine("bool {}::canReturnInRegs(MirType *type) const", className);
        {
            ...
            if (file.m_classification.m_aggregate.has_value())
            {
                for (const auto &cond : file.m_classification.m_aggregate->m_conditions)
                {
                    ...
                    {
                        if (cond.m_kind == DSL::Ast::CallingConvDef::AggregateCondition::Kind::SizeGreaterThan && cond.m_sizeLimit.has_value())
                        {
                            emitter.emitLine("if (type->getTotalSizeInBits() > {}) return false;", cond.m_sizeLimit->m_node * 8);
                        }
                    }
                }
            }
        ...
        // classify
        emitter.emitLine("void {}::classify(MirType *type, std::pmr::vector<CallingConvTypeClass> &out) const", className);
        {
            auto body = emitter.enterBlock();
            emitter.emitLine("if (!type) {{ out.push_back(CallingConvTypeClass::Integer); return; }}");
            emitter.emitLine("if (type->getKind() == MirTypeKind::FloatingPoint) {{ out.push_back(CallingConvTypeClass::Float); return; }}");
            emitter.emitLine("if (type->getKind() == MirTypeKind::Pointer || type->getKind() == MirTypeKind::Integer) {{ out.push_back(CallingConvTypeClass::Integer); return; }}");
        ...
        emitter.emitLine("ArgumentLocationDesc {}::getArgLoc(MirType *type, CallLoweringState *callState)", className);
        {
            ...
            emitter.emitLine("if (callState)");
            {
                ...
                if (!file.m_arguments.m_unifiedSlots.empty())
                {
                    ...
                    emitter.emitLine("switch (slot)");
                    {
                        ...
                        for (size_t s = 0; s < numSlots; ++s)
                        {
                            ...
                            {
                                emitter.emitLine("if (type && type->getKind() == MirTypeKind::FloatingPoint) return ArgumentLocationDesc::Reg(MirRegisterRef(m_defaultClass, resolveRegId(\"{}\", {})), sizeInBytes);",
                                    floatBinding->m_register.m_node, findRegId(floatBinding->m_register.m_node));
                                emitter.emitLine("return ArgumentLocationDesc::Reg(MirRegisterRef(m_defaultClass, resolveRegId(\"{}\", {})), sizeInBytes);",
                                    intBinding->m_register.m_node, findRegId(intBinding->m_register.m_node));
                            }
                            ...
                            {
                                emitter.emitLine("return ArgumentLocationDesc::Reg(MirRegisterRef(m_defaultClass, resolveRegId(\"{}\", {})), sizeInBytes);",
                                    slot.m_bindings[0].m_register.m_node, findRegId(slot.m_bindings[0].m_register.m_node));
                            }
                            else
                            {
                    ...
                    emitter.emitLine("return ArgumentLocationDesc::Stack(sizeInBytes, callState->allocateStack(type));");
                }
                else if (!file.m_arguments.m_rules.empty())
                {
                    ...
                    if (floatRule && std::holds_alternative<DSL::Ast::CallingConvDef::RegisterSequence>(floatRule->m_source))
                    {
                        ...
                        emitter.emitLine("if (type && type->getKind() == MirTypeKind::FloatingPoint)");
                        {
                            ...
                            emitter.emitLine("size_t cursor = callState->getBankCursor(\"{}\");", floatRule->m_abiClass.m_node);
                            ...
                            {
                                if (r > 0) sseRegsStr += ", ";
                                sseRegsStr += std::format("MirRegisterRef(m_defaultClass, resolveRegId(\"{}\", {}))", seq.m_registers[r].m_node, findRegId(seq.m_registers[r].m_node));
                            }
                    ...
                    if (intRule && std::holds_alternative<DSL::Ast::CallingConvDef::RegisterSequence>(intRule->m_source))
                    {
                        const auto &seq = std::get<DSL::Ast::CallingConvDef::RegisterSequence>(intRule->m_source);
                        emitter.emitLine("if (!type || type->getKind() == MirTypeKind::Integer || type->getKind() == MirTypeKind::Pointer)");
                        {
                            auto intBlock = emitter.enterBlock();
                            emitter.emitLine("size_t cursor = callState->getBankCursor(\"{}\");", intRule->m_abiClass.m_node);
                            emitter.emitLine("callState->advanceBankCursor(\"{}\");", intRule->m_abiClass.m_node);
                            std::string intRegsStr;
                            for (size_t r = 0; r < seq.m_registers.size(); ++r)
                            {
                                if (r > 0) intRegsStr += ", ";
                                intRegsStr += std::format("MirRegisterRef(m_defaultClass, resolveRegId(\"{}\", {}))", seq.m_registers[r].m_node, findRegId(seq.m_registers[r].m_node));
                            }
        ...
        // getReturnLoc
        emitter.emitLine("ArgumentLocationDesc {}::getReturnLoc(MirType *type, CallLoweringState *callState)", className);
        {
            auto body = emitter.enterBlock();
            emitter.emitLine("size_t sizeInBytes = (type ? type->getTotalSizeInBits() + 7 : 64) / 8;");
            size_t sretRegId = 0;
            ...
            emitter.emitLine("if (!canReturnInRegs(type))");
            {
                ...
                emitter.emitLine("return ArgumentLocationDesc::Indirect(false, true, sizeInBytes, MirRegisterRef(m_defaultClass, resolveRegId(\"{}\", {})));", sretRegName, sretRegId);
            }
            ...
            {
                emitter.emitLine("if (type && type->getKind() == MirTypeKind::FloatingPoint) return ArgumentLocationDesc::Reg(MirRegisterRef(m_defaultClass, resolveRegId(\"{}\", {})), sizeInBytes);", floatRetName, floatRetRegId);
            }
            emitter.emitLine("return ArgumentLocationDesc::Reg(MirRegisterRef(m_defaultClass, resolveRegId(\"{}\", {})), sizeInBytes);", intRetName, intRetRegId);
        }
        emitter.emitBlankLine();

        // Preserved register getters
        emitter.emitLine("const std::pmr::vector<MirRegisterRef> &{}::getAllCalleeSavedRegs() {{ return m_allCalleeSaved; }}", className);
        emitter.emitLine("const std::pmr::vector<MirRegisterRef> &{}::getCalleeSavedRegs(MirRegisterClass *) {{ return m_allCalleeSaved; }}", className);
        emitter.emitLine("const std::pmr::vector<MirRegisterRef> &{}::getAllCallerSavedRegs() {{ return m_allCallerSaved; }}", className);
        emitter.emitLine("const std::pmr::vector<MirRegisterRef> &{}::getCallerSavedRegs(MirRegisterClass *) {{ return m_allCallerSaved; }}", className);
        emitter.emitBlankLine();
    }
}
