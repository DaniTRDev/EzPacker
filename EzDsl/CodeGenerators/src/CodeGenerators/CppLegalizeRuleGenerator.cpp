...
{
    std::string guard = std::format("EZTRIPLE_{}_LEGALIZER_RULES_H", ToUpper(m_targetName));
    emitter.emitIncludeGuardStart(guard);

    emitter.emitLine("// Auto-generated by EzDSL CppLegalizeRuleGenerator. DO NOT EDIT.");
    emitter.emitBlankLine();

    emitter.emitInclude("Actions/LegalizeActionCommon.h", false);
...
void CppLegalizeRuleGenerator::emitSource(CppSourceEmitter &emitter) const
{
    ...
    emitter.emitInclude("Operand/MirInteger.h", false);
    emitter.emitInclude("Operand/MirRegister.h", false);
    ...
    for (const auto *r : rules)
    {
        ...
        for (size_t i = 0; i < matchInst.m_operands.size(); ++i)
        {
            ...
            if (op.m_kind == DSL::Ast::LegalizeRuleDef::RuleOperandKind::SsaRegister)
            {
                ...
                emitter.emitLine("if (!{} || {}->getType() != MirOperandType::Register) return LegalizationResult::NotModified;",
                                 varName, varName);
                ...
                if (op.m_typeOrClassId.has_value())
                {
                    if (const Symbol *tsSym = m_table->getSymById(*op.m_typeOrClassId))
                    {
                        if (const auto *ts = tsSym->getIf<Symbols::TypeSymbol>())
                        {
                            if (ts->m_bitWidth > 0)
                            {
                                emitter.emitLine("if ({}->getMirType()->getBitWidth() != {}) return LegalizationResult::NotModified;",
                                                 varName, ts->m_bitWidth);
                            }
                        }
                    }
                }
            ...
            else if (op.m_kind == DSL::Ast::LegalizeRuleDef::RuleOperandKind::ImmediateSymbol)
            {
                ...
                emitter.emitLine("if (!{} || {}->getType() != MirOperandType::Integer) return LegalizationResult::NotModified;",
                                 varName, varName);
            ...
            else if (op.m_kind == DSL::Ast::LegalizeRuleDef::RuleOperandKind::ImmediateLiteral)
            {
                ...
                emitter.emitLine("if (static_cast<MirInteger *>({})->getValue().getI64() != {}) return LegalizationResult::NotModified;",
                                 varName, litVal);
            }
        }
        emitter.emitBlankLine();
        ...
        for (size_t eIdx = 0; eIdx < r->m_expansionSequence.size(); ++eIdx)
        {
            ...
            for (size_t oIdx = 0; oIdx < e.m_operands.size(); ++oIdx)
            {
                ...
                if (op.m_kind == DSL::Ast::LegalizeRuleDef::RuleOperandKind::SsaRegister)
                {
                    ...
                    if (definedSsaVars.find(varName) == definedSsaVars.end())
                    {
                        ...
                        emitter.emitLine("MirOperand *op_{} = opBuilder.buildVReg(ctx.m_ctx->getTypeTable()->{}, \"{}\");",
                                         varName, typeAccessor, varName);
                ...
                else if (op.m_kind == DSL::Ast::LegalizeRuleDef::RuleOperandKind::CustomTransform)
                {
                    ...
                    if (!firstImmSymbolName.empty())
                    {
                        emitter.emitLine("MirOperand *{} = opBuilder.buildInt(imm_{}->getMirType(), FlexInt({}, imm_{}->getMirType()->getBitWidth()));",
                                         opName, firstImmSymbolName, transVar, firstImmSymbolName);
                    }
                    else
                    {
                        emitter.emitLine("MirOperand *{} = opBuilder.buildInt(ctx.m_ctx->getTypeTable()->i32(), FlexInt({}, 32));",
                                         opName, transVar);
                    }
            ...
            emitter.emitLine("builder.build(MirInstructionOpCode::{}, inst->getSourceReference(), {{ {} }});",
                             e.m_opcode, opList);
        }
        emitter.emitBlankLine();
        ...
        emitter.emitLine("block->erase(ctx.m_it);");
...
bool GenerateLegalizerRules(DiagnosticCollector *collector,
                           SymbolTable *table,
                           std::filesystem::path outPath,
                           std::string targetName)
{
    CppLegalizeRuleGenerator generator(collector, table, std::move(outPath), std::move(targetName));
