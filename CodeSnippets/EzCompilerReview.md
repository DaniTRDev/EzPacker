...
- [x] **P1 · DUP-02 — ELF/COFF writer setup duplicated**
...
- [x] **P1 · DUP-03 — Undefined-symbol creation duplicated**
  - Where: `EzCompiler/src/EmissionEngine.cpp:186-193` vs `:317-322`
  - Fix: one helper.
  - Status: fixed

- [x] **P1 · DUP-04 — Pass-pipeline scaffolding duplicated per stage**
...
- [x] **P1 · DUP-05 — `dumpCurrentMir` vs `dumpAssembly` duplicate the walk**
  - Where: `EzCompiler/src/CompilationPipeline.cpp:203-230` vs `:232-267`
  - Fix: one walker parameterized by the formatter.
  - Status: fixed

- [x] **P2 · DUP-06 — `CommandLineParser` mirrors the EzDsl/Cli parser and is set up twice**
...
- [x] **P2 · DUP-07 — Target ownership triplet duplicated and re-validated**
...
- [x] **P2 · DUP-08 — Arena construction written three ways**
...
- [x] **P1 · LEG-01 — `EmissionEngine::emitFunction` is declared but never defined/called**
...
- [x] **P1 · LEG-02 — `getFunctionAllocator`/`resetFunctionAllocator` unreachable**
...
- [x] **P1 · LEG-03 — Parsed options never consumed (user-visible dead surface)**
...
- [x] **P1 · LEG-04 — Dead public API / accessors**
...
- [x] **P1 · LEG-05 — `TargetTriple` predicates unused in production**
...
- [x] **P1 · LEG-06 — Stale includes in the target registration TU**
...
- [x] **P1 · LEG-07 — Dead alignment/code-model APIs, so functions are never aligned**
...
- [x] **P1 · LEG-08 — Placeholder frontend silently ignores non-MIR input**
...
- [x] **P2 · LEG-09 — `SectionType::Custom` emitted but unsupported by both writers**
...
- [x] **P1 · WEI-02 — Shared sections are finalized multiple times**
...
- [x] **P1 · WEI-03 — Emitter failures are silently swallowed**
...
- [x] **P1 · WEI-04 — Relocation type/section ignored; semantics inferred from opcode bytes**
...
- [x] **P1 · WEI-05 — Unchecked output writes / no atomic output**
...
- [x] **P1 · WEI-06 — `const_cast` span to satisfy the emitter API**
...
- [x] **P1 · WEI-07 — Emitter ownership via raw `new` across the seam**
...
- [x] **P1 · OPT-01 — O(n²) symbol-name lookup during relocation resolution**
  - Where: `EmissionEngine.cpp:306-314`
  - Fix: maintain an `unordered_set<string>` (or reuse `funcById`).
  - Status: fixed

- [x] **P1 · OPT-02 — `CodeSection::getCurrentOffset` is O(nodes) and called per symbol/function**
  - Where: `EzCodeEmitter/src/CodeSection.cpp:262-283`, callers `EmissionEngine.cpp`
  - Fix: maintain a running byte count.
  - Status: fixed

- [x] **P1 · OPT-03 — `MirPassManager` rebuilt per function per stage**
