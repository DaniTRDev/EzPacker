...
std::shared_ptr<ParseRule> base()
{
    return combinators::sequence(
               {tokenType(IRTokenType::LeftParen), virtualVariable(), tokenType(IRTokenType::RightParen)})
        ->map<MemoryNode>(IRMemoryReferenceType::Base);
}

std::shared_ptr<ParseRule> baseDispl()
{
    return combinators::sequence({tokenType(IRTokenType::LeftParen), virtualVariable(), tokenType(IRTokenType::Comma),
...
{
    return combinators::sequence({tokenType(IRTokenType::LeftParen), tokenType(IRTokenType::Comma), virtualVariable(),
...
{
    return combinators::anyOf({base(), baseDispl(), baseIndexScaleDisplacement(), direct(), indexScale()});
}
} // namespace grammar
