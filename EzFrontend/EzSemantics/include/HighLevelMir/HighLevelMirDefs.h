...
enum HighLevelMirFlags : uint32_t
{
    None = 0,
...
enum HighLevelMirOpCode : uint8_t
{
#define INSTRUCTION(name, operandCount, flags) name,
#include "HighLevelInstructionSet.h"
#undef INSTRUCTION
};

struct HighLevelMirMetadata
{
...
inline const HighLevelMirMetadata g_HighLevelMirInstructionSet[] = {
#define INSTRUCTION(name, operandCount, flags)                                                                         \
    HighLevelMirMetadata{ .m_opcode = name,                                                                            \
...
inline std::map<std::string, HighLevelMirOpCode> g_String2HighLevelMirInstruction = {
#define INSTRUCTION(name, operandCount, flags) { StrToLower(#name), name },
#include "HighLevelInstructionSet.h"
#undef INSTRUCTION
};

inline const HighLevelMirMetadata &getMeta(HighLevelMirOpCode op) { return g_HighLevelMirInstructionSet[op]; }

inline HighLevelMirOpCode getOpCodeFromStr(const std::string &str)
{
    auto it = g_String2HighLevelMirInstruction.find(StrToLower(str));

    if (it == g_String2HighLevelMirInstruction.end())
        return HighLevelMirOpCode::INVALID;
