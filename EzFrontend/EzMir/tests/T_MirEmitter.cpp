...
{
    MirRegister dst = emitter->createRegister(8);
    MirRegister src = emitter->createRegister(8);
...
{
    MirRegister r1 = emitter->createRegister(8);
    MirRegister r2 = emitter->createRegister(8);
...
{
    MirRegister r = emitter->createRegister(4);
...
{
    MirRegister a = emitter->createRegister(8);
    MirRegister b = emitter->createRegister(8);
...
TEST_F(MirEmitterTests, EmitJmpWithReference)
{
    ...
    MirInstruction *instr = emitter->emitJMP(MirOperand(MirReference{ target->getId() }));
    ASSERT_NE(instr, nullptr);
...
{
    MirRegister r = emitter->createRegister(8);
...
TEST_F(MirEmitterTests, InstructionMetadataOperandCount)
{
    ...
    EXPECT_EQ(nop->getMetadata().m_operandCount, 0u);

    MirRegister dst = emitter->createRegister(8);
    MirRegister src = emitter->createRegister(8);
    MirInstruction *mov = emitter->emitMOV(MirOperand(dst), MirOperand(src));
    ASSERT_NE(mov, nullptr);
    EXPECT_EQ(mov->getMetadata().m_operandCount, 2u);
}
...
{
    MirOperand op(MirReference{ 99 });
    EXPECT_EQ(op.getType(), MirOperandType::Reference);
    ASSERT_NE(op.getReference(), nullptr);
    EXPECT_EQ(op.getReference()->m_refId, 99u);
}

TEST_F(MirEmitterTests, MirOperandMemoryType)
{
    MirOperand op(MirMemory{ 1, 2, 4, 16 });
    EXPECT_EQ(op.getType(), MirOperandType::Memory);
    ASSERT_NE(op.getMemory(), nullptr);
    EXPECT_EQ(op.getMemory()->m_baseRegId, 1u);
    EXPECT_EQ(op.getMemory()->m_indexRegId, 2u);
    EXPECT_EQ(op.getMemory()->m_scale, 4);
    EXPECT_EQ(op.getMemory()->m_offset, 16);
}
...
TEST_F(MirEmitterTests, PushOperandToInstruction)
{
    ...
    ASSERT_NE(instr, nullptr);
    MirRegister r = emitter->createRegister(8);
    emitter->pushOperandToInstruction(instr, MirOperand(r));
    EXPECT_TRUE(instr->hasOperands());
...
{
    MirRegister r1 = emitter->createRegister(1);
    MirRegister r2 = emitter->createRegister(2);
    MirRegister r4 = emitter->createRegister(4);
    MirRegister r8 = emitter->createRegister(8);
