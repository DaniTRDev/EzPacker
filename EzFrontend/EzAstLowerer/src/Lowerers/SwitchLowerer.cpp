...
bool SwitchLowerer::lower(AstNode *node, AstLoweringContext *ctx)
{
    ...
    emitter->emitJMP(ctx->getEmitterContext()->createReference(currentCheckerBlock));
    ctx->enterSwitch(mergeBlock);
    {
        for (AstNode *switchCase : *_switch->getCases())
        {
            ...
            emitter->emitJE(ctx->getEmitterContext()->createReference(caseBodyBlock));
            // If not equal, jump to the NEXT checker block.
            emitter->emitJMP(ctx->getEmitterContext()->createReference(nextCheckerBlock));
            ...
            emitter->emitJMP(ctx->getEmitterContext()->createReference(mergeBlock));
