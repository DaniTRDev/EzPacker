...
bool AstLoweringContext::isSymbolLinkedToMir(Symbol *symbol) const
{
...
bool AstLoweringContext::linkTypeToMirTypeId(Type *semanticType, size_t mirTypeId)
{
...
std::pair<MirOperand, MirType *> AstLoweringContext::popOperand()
{
    if (!hasOperands())
    {
        throw std::runtime_error(
    ...
    std::pair<MirOperand, MirType *> pair = m_operandStack.top();
    m_operandStack.pop();
    return pair;
}
...
MirType *AstLoweringContext::createMirTypeFromSemanticType(Type *semanticType)
{
    ...
    switch (semanticType->getUnderlyingType())
    {
        ...
        {
            throw std::runtime_error(
                    "Internal Compiler Error: Don't know how to create a MIR type from semantic type ");
        }
    }

    TypedPoolLinkedList<MirType> *subTypes = nullptr;
    if (kind == MirTypeKind::Array)
    {
        subTypes = m_emitterContext->getTypePool()->createLinkedList<MirType>();
        for (auto &subType : semanticType->getSubTypes())
        {
            MirType *mirSubType =
                    createMirTypeFromSemanticType(subType); /*
                                                             ...
                                                             */
            m_emitterContext->getTypePool()->appendToListBack(subTypes, mirSubType);
        }
    }

    MirType *mirType = m_emitterContext->createType(kind, subTypes, semanticType->getTypeName());
    if (!linkTypeToMirTypeId(semanticType, mirType->getId()))
    {
...
void AstLoweringContext::pushInstruction(MirInstruction *instruction)
{
    m_instructionStack.push(std::move(instruction));
}

void AstLoweringContext::pushOperand(MirOperand operand, MirType *type)
{
    m_operandStack.push(std::make_pair(std::move(operand), type));
}
...
const std::shared_ptr<struct MirEmitterContext> &AstLoweringContext::getEmitterContext() const
{
    return m_emitterContext;
}
...
const std::stack<std::pair<MirOperand, MirType *>> &AstLoweringContext::getOperandStack() const
{
