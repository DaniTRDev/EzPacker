...
class AstLowererTests : public ::testing::Test
{
  protected:
    ...
    std::shared_ptr<AstLoweringContext> loweringCtx;
    ...
    void SetUp() override
    {
        ...
        loweringCtx = std::make_shared<AstLoweringContext>(semanticCtx, mirEmitter, mirCtx);
    ...
    std::shared_ptr<BasicParsingContext> makeParseCtx(const std::string &src)
    {
        ...
        ec->beginScope();
        {
            size_t id = sm->addSourceContent(std::format("test_{}", currId++), src);
    ...
    std::vector<Module *> parseModules(const std::string &src, size_t expectedNum)
    {
        AstNode *node = nullptr;
        size_t num = 0;
        ...
        ec->beginScope();
        {
            ...
            while (true)
            {
                ...
                num++;
                modules.push_back(dynamic_cast<Module *>(node));
            }
        }
...
TEST_F(AstLowererTests, FunctionCallWithArgumentsLowersCorrectly)
{
    std::vector<Module *> mods =
            parseModules("void callee(i64 %a, i64 %b) {} void caller() { call %callee(1, 2); }", 2);

    ASSERT_EQ(mods.size(), 2);

    ASSERT_NE(mods[0], nullptr);
    ASSERT_NE(mods[1], nullptr);

    ASSERT_TRUE(runSemanticPipeline(mods[0]));
    ASSERT_TRUE(runSemanticPipeline(mods[1]));

    ASSERT_TRUE(runLowering(mods[0]));
    ASSERT_TRUE(runLowering(mods[1]));
}

TEST_F(AstLowererTests, FunctionCallWithInvalidArgumentsLowersCorrectly)
{
    std::vector<Module *> mods = parseModules("void callee(i64 %a, i64 %b) {} void caller() { call %callee(1); }", 2);
    ...
    ASSERT_TRUE(runSemanticPipeline(mods[0]));
    ASSERT_FALSE(runSemanticPipeline(mods[1]));
}

TEST_F(AstLowererTests, MovFunctionAddressWrong)
{
    Module *mod = parseModule("void caller() { create i64 %vreg; mov i64 %vreg, i64 %caller;  }");
    ASSERT_NE(mod, nullptr);
    ASSERT_TRUE(runSemanticPipeline(mod));
    ASSERT_FALSE(runLowering(mod));
}