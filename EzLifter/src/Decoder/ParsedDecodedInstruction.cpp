...
bool ParsedDecodedInstruction::getFromParser(std::unique_ptr<IDecodedInstructionParser> parser, const std::vector<std::shared_ptr<IDecodedOperand>> &operands)
{
    ...
    {
        LogSink::pushLog(
            LogMessage("Could not getFromParser instruction because AstNodeParser is invalid").colorize(Colors::red));
        return false;
    }
    ...
    {
        LogSink::pushLog(
            LogMessage("Could not getFromParser instruction because operands are invalid").colorize(Colors::red));
        return false;
    }
