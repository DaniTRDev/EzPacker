...
TEST_F(MirTypeLegalizerTests, NoChangesForLegalSizedRegisters)
{
    ...
    MirBlock* block = emitterCtx->getBlocks()->get<MirBlock>(0); // createFunction gives entry block
...
TEST_F(MirTypeLegalizerTests, ExpandLoadWithOffset)
{
    ...
    // We can emit load directly or simulate it. We will emit instructions manually if emitter emitLOAD doesn't take 3 operands
    ...
    MirInstruction* loadInstr = emitterCtx->getInstructionPool()->create<MirInstruction>(MirInstructionOpCode::LOAD, loadOps);
